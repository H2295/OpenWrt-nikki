...
const callnikkiboxVersion = rpc.declare({
    object: 'luci.nikkibox',
    method: 'version',
    expect: { '': {} }
});

const callnikkiboxUpdateSubscription = rpc.declare({
    object: 'luci.nikkibox',
...
const callnikkiboxGetIdentifiers = rpc.declare({
    object: 'luci.nikkibox',
    method: 'get_identifiers',
    expect: { '': {} }
});

const callnikkiboxUpgrade = rpc.declare({
    object: 'luci.nikkibox',
    method: 'upgrade',
    expect: { '': {} }
});

const homeDir = '/etc/nikkibox';
...
return baseclass.extend({
    ...
    status: async function () {
        return (await callRCList('nikkibox'))?.nikkibox?.running;
    },

    reload: function () {
        return callRCInit('nikkibox', 'reload');
    },

    restart: function () {
        return callRCInit('nikkibox', 'restart');
    },

    version: function () {
        return callnikkiboxVersion();
    },

    getnikkiboxConfig: function() {
        return L.resolveDefault(fs.read_direct('/etc/nikkibox/run/config.json')).then(function(res) {
            try {
    ...
    updateSubscription: function (section_id) {
        return callnikkiboxUpdateSubscription(section_id);
    },

    api: async function (method, path, query, body) {
        const config = await this.getnikkiboxConfig();
    ...
    getIdentifiers: function () {
        return callnikkiboxGetIdentifiers();
    },
